...
#    define IPC_BUFFER_EXT \
        ((uint16_t) 400U) // size addded to RX buffer because of RX queue
                          // implementation (using headers for messages)
#    define IPC_RXBUF_MAXSIZE \
        ((uint16_t) 1600U     \
         + IPC_BUFFER_EXT) // maximun size of character queue size has to match
                           // ATCMD_MAX_CMD_SIZE
...
#        define IPC_RXBUF_STREAM_MAXSIZE \
            ((uint16_t) IPC_RXBUF_MAXSIZE) // maximum size of stream queue (if
                                           // used)
