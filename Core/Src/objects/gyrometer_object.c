...
static three_axis_sensor_object_t g_gyrometer_obj;

int gyrometer_object_install(anjay_t *anjay) {
    return three_axis_sensor_object_install(anjay, &IKS01A2_GYROMETER_DRIVER,
                                            3334, &g_gyrometer_obj);
}

void gyrometer_object_update(anjay_t *anjay) {
    three_axis_sensor_object_update(anjay, &g_gyrometer_obj);
}
