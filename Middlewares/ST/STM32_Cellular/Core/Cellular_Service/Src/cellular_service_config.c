...
#define CST_DEFAULT_PARAMA_NB     15U                  /* Cellular parameter nb */

#define CST_SETUP_NFMC       (1)          /* NMFC code included */

#define CST_LABEL "Cellular Service"      /* Setup munu label */
...
static void CST_setup_dump(void)
{
  ...
  /* NFMC actived, display all NFMC values */
...
uint32_t CST_update_config_setup_handler(dc_apn_config_t *apn_config, dc_cs_sim_slot_type_t  active_slot)
{
  ...
  {
    /* inconsistant nb stot parameter */
    sim_slot_nb = 0U;
  }
